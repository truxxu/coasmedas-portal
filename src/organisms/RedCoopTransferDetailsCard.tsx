...
export function RedCoopTransferDetailsCard({
  sourceAccounts,
  destinationAccounts,
  selectedSourceId,
  ...
  hideBalances,
  error,
}: RedCoopTransferDetailsCardProps) {
  const formatSourceOption = (account: RedCoopSourceAccount) => {
  ...
  const formatDestinationOption = (account: RedCoopDestinationAccount) => {
    return `${account.holderName} - ${account.accountNumber}`;
  };
      ...
      <div className="space-y-5">
        ...
        <div>
          <label
            ...
            className="block text-[14px] font-medium text-brand-text-black mb-2"
          >
            Cuenta destino / Codigo de producto
          </label>
          {hasNoDestinations ? (
