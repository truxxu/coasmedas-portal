...
export default function ActivarCodigoSmsPage() {
  ...
  const {
    ...
    // TODO: replace mock with real service once the tarjeta activación endpoint
    // is available.
    onSubmit: async () => {
      ...
      const confirmation: TarjetaActivacionConfirmationData =
        JSON.parse(confirmationStr);

      await new Promise((resolve) => setTimeout(resolve, 800));

      const result: TarjetaActivacionResult = {
        status: "success",
      ...
      sessionStorage.setItem("tarjetaActivacionResult", JSON.stringify(result));
    },
    onResend: async () => {
  ...
  useEffect(() => {
    setWelcomeBar({
      ...
      backHref: "/tarjeta/bloqueo-activacion/activar/confirmacion",
    });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="-mx-8 bg-white shadow-sm">
        <Stepper currentStep={3} steps={TARJETA_ACTIVACION_STEPS} />
      </div>
