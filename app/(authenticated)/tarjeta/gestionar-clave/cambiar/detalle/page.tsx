...
const FORM_ID = "tarjeta-clave-cambiar-detalle-form";

function CambiarDetalleContent() {
  ...
  useEffect(() => {
    setWelcomeBar({
      title: "Cambiar Clave",
      backHref: "/tarjeta/gestionar-clave/cambiar",
    });
  ...
  const handleSubmit = (data: TarjetaClaveCambiarFormData) => {
    sessionStorage.setItem("tarjetaClaveCambiarCardId", product.id);
    sessionStorage.setItem(
      "tarjetaClaveCambiarProduct",
      JSON.stringify(product),
    );
    sessionStorage.setItem("tarjetaClaveCambiarForm", JSON.stringify(data));
...
export default function CambiarDetallePage() {
  return (
    <Suspense fallback={null}>
      <CambiarDetalleContent />
    </Suspense>
  );
}
